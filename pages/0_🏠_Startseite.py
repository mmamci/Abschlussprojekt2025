...
def build_main_page():
    ...
    with row1_col1:
        st.markdown("<h2 style='font-size:48px'>➕</h2>",
                    unsafe_allow_html=True)
        st.subheader("Variable erstellen")
        st.write(
            "Erstelle individuelle Variablen wie Schritte, Wasserzufuhr, Schlaf usw.")

    with row1_col2:
        st.markdown("<h2 style='font-size:48px'>✍️</h2>",
                    unsafe_allow_html=True)
        st.subheader("Werte eintragen")
        st.write(
            "Trage tägliche Werte zu deinen Variablen ein, um deinen Fortschritt zu verfolgen.")
    ...
    st.write(
        "Besondere Erfolge und Entwicklungen deiner Gesundheitsreise auf einen Blick.")
